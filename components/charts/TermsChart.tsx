  ...
  return (
    <Card className="bg-card border-border">
      <CardHeader className="pb-2">
        <CardTitle className="text-foreground text-[15px]">Best performing terms</CardTitle>
        <CardDescription className="text-muted-foreground text-[13px]">Terms correlated with high-engagement posts</CardDescription>
      </CardHeader>
      <CardContent className="px-6">
        {loading ? (
          <div className="grid grid-cols-2 gap-x-8 gap-y-3">
            {Array.from({ length: 12 }).map((_, i) => (
              <Skeleton key={i} className="h-4 w-full bg-muted rounded" />
            ))}
          </div>
        ) : (
          <div className="grid grid-cols-2 gap-x-8 gap-y-4">
            {terms.map((t) => (
              <div key={t.id} className="flex items-center gap-3">
                <span className="text-[13px] text-foreground/80 w-28 truncate">{t.term}</span>
