...
// adjacent hues are still easy to tell apart at small dot sizes.
function clusterColor(label: string, allLabels: string[]): string {
  const idx   = allLabels.indexOf(label)
  const total = allLabels.length
  const hue   = Math.round((idx / total) * 360)
  // Alternate between two lightness bands so neighbouring hues contrast
  const lightness = idx % 2 === 0 ? 55 : 70
