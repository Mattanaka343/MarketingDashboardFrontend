...
import { FilterProps, Metric, Post } from "@/types"
...
export default function TopPosts({ brand, channel, period }: FilterProps) {
  ...
  useEffect(() => {
    setLoading(true)
    fetchTopPosts({ brand, channel, period, limit: 6, metric})
      .then(setPosts)
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="text-[15px] font-medium text-foreground">Top posts</p>
          <p className="text-[13px] text-muted-foreground">Ranked by {metricLabel.toLowerCase()}</p>
        </div>
          ...
          <Card className="bg-card border-border h-full group-hover:border-border transition-colors">
            <CardContent className="p-4 flex flex-col gap-3 h-full">
              ...
              <div className="flex gap-4 pt-2 border-t border-border mt-auto">
                ...
                <Stat label="Eng."        value={post.engagement_rate != null ? `${post.engagement_rate.toFixed(1)}%` : "—"} />
              </div>
            </CardContent>
          </Card>
        </a>
